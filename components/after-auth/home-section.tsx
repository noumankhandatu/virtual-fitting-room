...
const categories = [
  { label: "MENS TOPS", path: "/all-items/male/upper_body" },
  { label: "MENS BOTTOMS", path: "/all-items/male/lower_body" },
  { label: "WOMEN TOPS", path: "/all-items/female/upper_body" },
  { label: "WOMEN BOTTOMS", path: "/all-items/female/lower_body" },
];

const CategoryButton = ({ label, path }: { label: string; path: string }) => (
  <Link href={path}>
      ...
      <div className="relative h-full flex items-end justify-center z-20 pb-[150px]">
        <div className="grid grid-cols-2 gap-4 sm:flex sm:gap-6">
          {categories.map((category) => (
            <CategoryButton key={category.label} label={category.label} path={category.path} />
          ))}
        </div>
      </div>
